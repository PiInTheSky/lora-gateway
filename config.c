...
RegisterConfigFile( char *Filename ) 
{
    ConfigFilename = Filename;
}  int

RegisterConfigSection( char *Section ) 
{
...
FindSettingIndex( int SectionIndex, int Index, char *Name ) 
{
...
RegisterConfigDouble( int SectionIndex, int Index, char *Name,
                      double *DoubleValuePtr, void ( Callback ) ( int ) ) 
{
        ...
        
    {
                  ...
                  FindSettingIndex( SectionIndex, Index, Name ) ) >= 0 )
...
RegisterConfigBoolean( int SectionIndex, int Index, char *Name,
                       int *BoolValuePtr, void ( Callback ) ( int ) ) 
{
...
RegisterConfigString( int SectionIndex, int Index, char *Name,
                      char *StringValuePtr, int MaxValueLength,
                      void ( Callback ) ( int ) ) 
{
...
GetLongName( int SettingIndex, char *ValueName, int Length ) 
{
    ...
    {
        sprintf( ValueName, "%s%s_%d", Sections[Setting->SectionIndex],
                  Setting->ValueName, Setting->Index );
    }
    
    else
        
    {
        sprintf( ValueName, "%s%s", Sections[Setting->SectionIndex],
                  Setting->ValueName );
    }
}

 int
ReadConfigValue( int SettingIndex ) 
{
            ...
            
        {
                ...
                
            {
                    ...
                    
                {
                        ...
                        strncpy( Setting->StringValuePtr, temp,
                                  Setting->MaxValueLength - 1 );
...
FindSettingByName( char *ValueNameToFind ) 
{
                ...
                
            {
                    ...
                    
                {
                        ...
                        
                    {
                                      ...
                                      *( Setting->DoubleValuePtr ) );
                            break;
                         case stBoolean:
                            fprintf( dest, "%s=%c\n", token,
                                      *( Setting->IntValuePtr ) ? 'Y' : 'N' );
...
SetConfigValue( char *Setting, char *Value ) 
{
        ...
        
    {
            ...
            
        {
                ...
                strncpy( Settings[SettingIndex].StringValuePtr, Value,
                          Settings[SettingIndex].MaxValueLength - 1 );
                ...
                *Settings[SettingIndex].IntValuePtr =
                    Value[strcspn( Value, "1YyTt" )];
...
SettingAsString( int SettingIndex, char *SettingName, int SettingNameSize,
                 char *SettingValue, int SettingValueSize ) 
{
        ...
        
    {
            ...
            GetLongName( SettingIndex, SettingName,
                          sizeof( SettingNameSize ) );
         switch ( Settings[SettingIndex].SettingType )
            
        {
            case stString:
                snprintf( SettingValue, SettingValueSize - 1, "\"%s\"",
                           Settings[SettingIndex].StringValuePtr );
                break;
             case stInteger:
                snprintf( SettingValue, SettingValueSize - 1, "%d",
                           *Settings[SettingIndex].IntValuePtr );
                break;
             case stDouble:
                snprintf( SettingValue, SettingValueSize - 1, "%lf",
                           *Settings[SettingIndex].DoubleValuePtr );
                break;
             case stBoolean:
                snprintf( SettingValue, SettingValueSize - 1, "%d",
                           *Settings[SettingIndex].IntValuePtr ? 1 : 0 );
