...
void
ProcessJSONClientLine( int connfd, char *line )
{
    line[strcspn( line, "\r\n" )] = '\0';   // Get rid of CR LF
    ...
    if ( strchr( line, '=' ) == NULL )
    {
        ...
        if ( strcasecmp( line, "settings" ) == 0 )
        {
            ...
            while ( SettingAsString
                    ( Index, SettingName, sizeof( SettingName ), SettingValue,
                      sizeof( SettingValue ) ) )
            {
                char temp[300];

                sprintf( temp,
                         "{\"class\":\"SET\",\"set\":\"%s\",\"val\":%s}\r\n",
                         SettingName, SettingValue );

                if ( ( strlen( temp ) + strlen( packet ) ) <
                     sizeof( packet ) )
                {
                    strcat( packet, temp );
                }
...
SendJSON( int connfd )
{
    ...
    for ( PayloadIndex = 0; PayloadIndex < MAX_PAYLOADS; PayloadIndex++ )
    {
        ...
        {
            sprintf( sendBuff,
                     "{\"class\":\"POSN\",\"index\":%d,\"channel\":%d,\"payload\":\"%s\",\"time\":\"%s\",\"lat\":%.5lf,\"lon\":%.5lf,\"alt\":%d,\"rate\":%.1lf}\r\n",
                     PayloadIndex, Config.Payloads[PayloadIndex].Channel,
            ...
            if ( !run )
            {
                port_closed = 1;
                      ...
                      ( connfd, sendBuff, strlen( sendBuff ),
                        MSG_NOSIGNAL ) <= 0 )
            {
...
ServerLoop( void *some_void_ptr )
{
    ...
    if ( setsockopt( ServerInfo->sockfd, SOL_SOCKET, SO_REUSEADDR, &( int )
                     {
                     1}, sizeof( int ) ) < 0 )
    {
    ...
    if ( bind
         ( ServerInfo->sockfd, ( struct sockaddr * ) &serv_addr,
           sizeof( serv_addr ) ) < 0 )
    {
    ...
    while ( run )
    {
        ...
        while ( ServerInfo->Connected )
        {
            ...
            while ( ( bytecount =
                      recv( connfd, packet, sizeof( packet ), 0 ) ) > 0 )
            {
                char *line, *saveptr;
