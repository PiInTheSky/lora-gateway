                   ...
                   int LowDataRateOptimize)
{
    ...
    ChannelPrintf (Channel, 2, 1, "%s, %s, SF%d, EC4:%d %s",
                   ...
                   (ErrorCoding >> 1) + 4, LowDataRateOptimize ? "LDRO" : "");
}
...
SendLoRaData (int Channel, char *buffer, int Length)
{
    ...
    writeRegister (Channel, REG_PAYLOAD_LENGTH,
                   Config.LoRaDevices[Channel].
                   PayloadLength ? Config.LoRaDevices[Channel].
...
DoPositionCalcs (Channel)
{
        ...
        && (Now > Config.LoRaDevices[Channel].LastPositionAt))
    {
        ...
        Period =
            (float) Now - (float) Config.LoRaDevices[Channel].LastPositionAt;
...
ProcessLine (int Channel, char *Line)
{
    ...
    if (Config.EnableDev)
    {
        sscanf (Line + 2,
                ...
                &(Config.LoRaDevices[Channel].Heading), &Satellites, &TempInt,
                &TempExt, &(Config.LoRaDevices[Channel].cda),
...
void
ReadString (FILE * fp, char *keyword, char *Result, int Length, int NeedValue)
{
...
LoadConfigFile ()
{
    ...
    ReadString (fp, "SMSFolder", Config.SMSFolder, sizeof (Config.SMSFolder),
                0);
    if (Config.SMSFolder[0])
    {
    ...
    for (Channel = 0; Channel <= 1; Channel++)
    {
        ...
        if (Config.LoRaDevices[Channel].Frequency[0])
        {
            ...
            Config.LoRaDevices[Channel].Power =
                ReadInteger (fp, Keyword, 0, PA_MAX_UK);
...
SendUplinkMessage (int Channel)
{
    ...
    else if (GetExternalListOfMissingSSDVPackets (Channel, Message))
    {
        ...
        SetLoRaParameters (Channel, EXPLICIT_MODE, ERROR_CODING_4_8,
                           BANDWIDTH_125K, SPREADING_8, 0);
        SendLoRaData (Channel, Message, 255);
    }
}
